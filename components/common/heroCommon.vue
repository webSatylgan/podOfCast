<script setup>
    // imports ----------------
    import {useSlots, computed} from "vue";

    // props ---------------------
    const props = defineProps({
       paddingBottom: [Number, String],
        hasContainer: {
           type: Boolean,
           default: true,
        }
    });

    // vars --------------
    const slots = useSlots();
    const hasSlotTitle = computed(() => !!slots.title)
</script>

<template>
    <div class="hero" :style="{paddingBottom: paddingBottom}">
        <div :class="{'container': hasContainer}">
            <div class="hero__title" v-if="hasSlotTitle">
                <slot name="title"></slot>
            </div>
            <slot></slot>
        </div>
        <div class="hero__effects flex">
            <svg class="hero__effect-left" viewBox="0 0 326 593" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M-104.953 83.9508C-104.953 130.457 -86.9684 208.876 -154.883 293.648C-221.219 376.45 -129.959 114.254 -38.3047 98.0256C68.8327 79.0554 -115.807 518.301 -125.328 428.909C-130.654 378.898 24.3181 116.399 67.9096 159.991C123.364 215.445 -22.0467 532.191 -56.0005 526.981C-115.9 517.791 98.3315 153.446 159.118 184.826C231.603 222.244 112.128 471.949 56.6898 485.343C12.339 496.057 177.755 254.065 288.446 309.449" stroke="black" stroke-width="8" stroke-linecap="round" />
                <path d="M295.941 289.989C297.022 288.054 299.98 289.052 299.667 291.246L299.091 295.288C298.904 296.605 300.029 297.734 301.346 297.552L317.061 295.372C319.181 295.078 320.219 297.871 318.421 299.033L306.744 306.583C305.389 307.459 305.576 309.496 307.067 310.111L315.016 313.393C316.844 314.148 316.585 316.813 314.647 317.203L309.444 318.248C308.269 318.484 307.564 319.694 307.938 320.833L309.414 325.327C309.978 327.044 308.156 328.569 306.565 327.711L300.221 324.291C298.593 323.413 296.749 325.026 297.4 326.756L300.89 336.022C301.647 338.032 299.135 339.66 297.61 338.147L287.746 328.364C287.11 327.733 286.132 327.602 285.353 328.043L274.234 334.334C272.381 335.382 270.391 333.195 271.609 331.449L278.157 322.062C278.756 321.204 278.592 320.028 277.78 319.367L266.362 310.073C264.903 308.885 265.743 306.522 267.625 306.522H276.421C277.415 306.522 278.258 305.791 278.4 304.807L279.779 295.245C280.078 293.176 282.949 292.894 283.645 294.864L285.879 301.188C286.452 302.811 288.671 303 289.511 301.497L295.941 289.989Z" fill="#CD4631" />
            </svg>
            <img class="hero__effect-right" src="@/assets/imgs/hero-stars.png" alt="">
        </div>
    </div>
</template>

<style lang="scss">
    .hero {
        width: 100%;
        padding-top: 90px;

        background: $pink-bg;

        position: relative;


        button {
            padding: 24px 48px;
        }

        button.tag {
            padding: 20px 36px;

            border: 2px solid $black;
            border-radius: 8px;

            font-weight: 700;
            font-size: .875rem;
            color: $black;
        }


        &__title {
            width: 100%;
            max-width: 765px;
            margin: 65px auto 0;

            text-align: center;

            h1 {
                span {
                    color: $red;
                }
            }

            p {
                margin: 40px auto 0;

                color: $graphite;
                font-size: 1rem;
            }
        }

        &__effects {
            align-items: flex-start;
            width: 100%;

            position: absolute;
            top: 0;
            left: 0;
            pointer-events: none;

            .hero__effect-left {
                max-width: 320px;
            }

            .hero__effect-right {
                align-self: center;
                max-width: 120px;
            }

        }
    }


    /* adaptive -------------------------- */
    @media (min-width: 1920px) {
        .hero {
            max-width: 1920px;
        }
    }
    
    @media (max-width: 1000px) {
        .hero__effects {
            display: none;
        }
    }
</style>